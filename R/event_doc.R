...
#' manually, but is fired as part of the normal lifetime of a \code{fiery} 
#' server:
#' 
#' \describe{
...
#' }
#' 
#' Additional parameters passed on to the \code{trigger()} method will be passed
#' on to the handler. There is no limit to the number of handlers that can be 
...
#' communicate with it using the \code{trigger()} method. Instead it is possible
#' to assign a directory to look in for event trigger instructions. The trigger
#' directory is set using the \code{triggerDir} field, e.g.:
#' 
#' \code{
#' app$triggerDir <- '/some/path/to/dir/'
#' }
#' 
#' Events are triggered by placing an \code{rds} file named after the event in
#' the trigger directory. The file must contain a list, and the elements of the
#' list will be passed on as arguments to the event handlers. After the event 
#' has been triggered the file will be deleted. The following command will 
#' trigger the \code{external-event} on a server looking in 
#' \code{'/some/path/to/dir/'}:
#' 
#' \code{
...
#' 
#' \code{\link{plugins}} describes how to use plugins to modify the server
#' 
#' @rdname event_doc
#' @name event_doc
#' @aliases events
